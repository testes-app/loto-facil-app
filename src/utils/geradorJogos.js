...
export function gerarComIA(historico) {
  ...
  // 1. Adiciona números quentes (misturando para dar variedade)
  const qtdQuentes = 6;
  const quentesAleat = [...quentes].sort(() => Math.random() - 0.5);
  jogo.push(...quentesAleat.slice(0, qtdQuentes));

  // 2. Adiciona números médios (misturando para dar variedade)
  const qtdMedios = 6;
  const mediosAleat = medios
    .filter(n => !jogo.includes(n))
    .sort(() => Math.random() - 0.5);
  jogo.push(...mediosAleat.slice(0, qtdMedios));

  // 3. Adiciona números frios para surpreender (20%)
  const qtdFrios = 3;
  const friosAleat = frios
    .filter(n => !jogo.includes(n))
    .sort(() => Math.random() - 0.5);
  jogo.push(...friosAleat.slice(0, qtdFrios));
